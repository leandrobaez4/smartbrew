  ...
  try {
    const response = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({ from: config.from, to: [email], subject: 'Tu invitación a SmartBrew',
        text: `Te invitaron al portal privado de SmartBrew.\n\nAbrí ${config.loginUrl} y elegí “Aceptar invitación”.\nIngresá este correo, el siguiente código y elegí una contraseña nueva de SmartBrew (mínimo 12 caracteres).\n\n${code}\n\nEl código vence en 7 días y solo se puede usar una vez. No ingreses tu contraseña de Instagram en SmartBrew. Después podrás conectar Instagram mediante su pantalla oficial.\n\nSi no esperabas esta invitación, ignorá este mensaje.` }),
    });
