...
import MetaReviewForm from './meta-review-form';
import { revokeMember } from './actions';
export const dynamic = 'force-dynamic';
export default async function InstagramAdmin() {
  await requireAdmin();
  const members = await portalDb.portalMember.findMany({ select: { id: true, email: true, disabled: true, reviewExpiresAt: true, inviteExpiresAt: true, createdAt: true, connection: { select: { username: true, instagramId: true, expiresAt: true, updatedAt: true } } }, orderBy: { createdAt: 'desc' } });
  ...
  return <section className="max-w-6xl space-y-6">
    ...
    {members.map(m => <div key={m.id} className="border rounded p-4 flex flex-wrap justify-between gap-3"><span>{m.reviewExpiresAt ? `Revisión Meta · ${m.id} · Vence ${date(m.reviewExpiresAt)}` : m.email} · {m.disabled ? 'Revocado' : m.reviewExpiresAt && m.reviewExpiresAt <= new Date() ? 'Vencido' : 'Habilitado'}</span>
